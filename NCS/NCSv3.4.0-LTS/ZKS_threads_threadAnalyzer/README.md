...
There are various ways to use the Thread Analyzer. The analysis is performed on demand when the application calls <code>thread_analyzer_run()</code> or <code>thread_analyzer_print()</code>. Alternatively, it is also possible to output the current statistics at regular intervals. We will use this second method here.
...
2) We add the Thread Analyzer by adding following KCONFIG setting in prj.conf.
