# Bluetooth Low Energy: Peripheral with a user-defined Service (Custom Service) - _Write_ (with Acknowledge)
...
In this hands-on we use the "Write" (with Acknowledge) transfer operation. A smartphone takes over the role as the GATT Client, and a development kit (e.g. nRF54L15DK) is used as the GATT Server. The Smartphone writes data into the GATT database. 
...
5) Add the declaration of CustomService_init() function to header file __CustomService.h__:
	...
	<sup>_services/CustomService.h_</sup>
       ...
       #ifndef CUSTOMSERVICE_H_
       #define CUSTOMSERVICE_H_

       void CustomService_init(void);
   
       #endif

7) We need a UUID for the custom service and also for the custom feature. Create two UUIDs at https://www.uuidgenerator.net. And add them to CusomtService.c:
  
	<sup>_services/CustomService.c_</sup>

       /*Note that UUIDs have Least Significant Byte ordering */
       #define CUSTOM_SERVICE_UUID        0xD0, 0x86, 0x48, 0x24, 0x54, 0xB3, 0x43, 0xA1, 0xBC, 0x20, 0x97, 0x8F, 0xC3, 0x76, 0xC2, 0x75                       
       #define CUSTOM_CHARACTERISTIC_UUID 0xD1, 0xAA, 0x20, 0x11, 0x92, 0xE7, 0x43, 0x5A, 0xAA, 0xE9, 0x94, 0x43, 0x35, 0x6A, 0xD4, 0xD3
...
8) The custom UUIDs have to be declared. Add following lines in CustomService.c:
...
9) We need to define and register our service and its characteristics. By using the following helper macro we statically register our Service in our BLE host stack.
...
10) Now add the function that takes care about getting a trigger if data is received. Add the following to CustomService.c just before the __BT_GATT_SERVICE_DEFINE()__ macro is called. This is needed, because the macro defines to call __on_receive()__ function!
	...
	<sup>_services/CustomService.c_</sup>
        ...
        ssize_t write_cb(struct bt_conn *conn,
